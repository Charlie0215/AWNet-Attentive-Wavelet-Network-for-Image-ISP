...
from utils import validation, print_log, to_psnr, adjust_learning_rate_step
...
def train():
    ...
    train_dataset = LoadData(
        trainConfig.data_dir, TRAIN_SIZE, dslr_scale=224, test=False)
    ...
    test_dataset = LoadData(
        trainConfig.data_dir, TEST_SIZE, dslr_scale=224, test=True)
    ...
    for epoch in range(trainConfig.epoch):
        ...
        torch.save(state, '{}/three_channel_epoch_{}.pkl'.format(
        ...
        if val_psnr >= pre_psnr:
            ...
            torch.save(state, '{}/weight_3channel_best.pkl'.format(
