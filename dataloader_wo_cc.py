...
class LoadData(Dataset):
    ...
    def __init__(self, dataset_dir, dataset_size, dslr_scale, test=False):
        ...
        if test:
            ...
            self.dslr_dir = os.path.join(dataset_dir, 'test', 'huawei_visualized')
            self.dataset_size = dataset_size
        else:
            self.raw_dir = os.path.join(dataset_dir, 'train', 'huawei_raw')
            self.dslr_dir = os.path.join(dataset_dir, 'train', 'huawei_visualized')
    ...
    def __getitem__(self, idx):
        ...
        dslr_image = np.float32(
            np.array(Image.fromarray(dslr_image).resize((dslr_img_shape[0], dslr_img_shape[1])))) / 255.0
