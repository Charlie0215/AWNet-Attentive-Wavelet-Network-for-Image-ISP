...
from models_v1.model import UNet#, PixelDiscriminator
from loss import Loss
from dataloader_wo_cc import LoadData, LoadVisualData
...
def train():
    ...
    loss = Loss().to(device)
    net = UNet(n_channels=4, n_classes=3).to(device)
    net = nn.DataParallel(net, device_ids=device_ids)

    # Reload
    if trainConfig.pretrain == True:
        net.load_state_dict(torch.load('{}/texture_best.pkl'.format(trainConfig.save_best))["model_state"])
    ...
    pre_psnr, pre_ssim = 0, 0#validation(net, test_loader, device, save_tag=True, mode='texture')
    print(
    ...
    for epoch in range(trainConfig.epoch):
        ...
        torch.save(state, '{}/texture_epoch_{}.pkl'.format(trainConfig.checkpoints, epoch))
        
        one_epoch_time = time.time() - start_time
        print('time: {}, train psnr: {}'.format(one_epoch_time, train_psnr))
        val_psnr, val_ssim = validation(net, test_loader, device, save_tag=True, mode='texture')
        print_log(epoch+1, trainConfig.epoch, one_epoch_time, train_psnr, val_psnr, val_ssim, 'texture')
        ...
        if val_psnr >= pre_psnr:
            ...
            torch.save(state, '{}/texture_best.pkl'.format(trainConfig.save_best))
            pre_psnr = val_psnr
