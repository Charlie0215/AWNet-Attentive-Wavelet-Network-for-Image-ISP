...
class LoadData(Dataset):
    ...
    def __init__(self, dataset_dir, dataset_size, dslr_scale, test=False):
        ...
        if test:
            self.inp_img = os.path.join(dataset_dir, 'test', 'canon')
            self.dslr_dir = os.path.join(dataset_dir, 'test', 'canon')
            self.dataset_size = dataset_size
        else:
            self.inp_img = os.path.join(dataset_dir, 'train', 'canon')
    ...
    def __getitem__(self, idx):
        ...
        input_img = imageio.imread(os.path.join(self.inp_img, str(idx) + ".jpg"))
        input_img = np.asarray(input_img)
        input_img_shape = input_img.shape
        input_img = np.float32(
            np.array(Image.fromarray(input_img).resize((input_img_shape[0]//self.scale, input_img_shape[1]//self.scale)))) / 255.0
        ...
        return input_img, dslr_image, str(idx)
