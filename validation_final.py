...
class LoadData_real(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tehnsor, str]:
        ...
        if self.is_ensemble:
            ...
            raw_image1 = [torch.from_numpy(x.transpose((2, 0, 1)).copy()) for x in raw_image1]  # type: ignore

            raw_image2 = ensemble_pillow(raw_image2)
            raw_image2 = [self.toTensor(x) for x in raw_image2]

        else:
            raw_image1 = torch.from_numpy(raw_image1.transpose((2, 0, 1)).copy())  # type: ignore
...
def test() -> None:
    ...
    net1.load_state_dict(
        torch.load(f"{trainConfig.save_best}/weight_4channel_best.pkl", map_location="cuda:0")["model_state"]
    )  # type: ignore
    net2.load_state_dict(
        torch.load("{trainConfig.save_best}/weight_3channel_best.pkl", map_location="cuda:0")["model_state"]
    )  # type: ignore
    print("weight loaded.")

    test_dataset = LoadData_real(trainConfig.data_dir, is_ensemble=ENSEMBLE)
    test_loader = DataLoader(
        dataset=test_dataset, batch_size=4, shuffle=False, num_workers=12, pin_memory=True, drop_last=False
    )
